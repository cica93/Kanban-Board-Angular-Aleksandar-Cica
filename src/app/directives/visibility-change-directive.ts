...
export class VisiblilityChangeDirective implements OnDestroy {
  ...
  appVisiblilityChange = output<boolean>();
    ...
    combineLatest([
      ...
      .subscribe(([rootMargin, threshold, root]) => {
        this.observer = subscribeOnElementAppear({
          ...
          threshold,
          root,
          callBackFunction: (isIntersecting: boolean) => {
            this.appVisiblilityChange.emit(isIntersecting);
          },
...
export function subscribeOnElementAppear(config: {
  ...
  callBackFunction: (isIntersecting: boolean) => void;
}): IntersectionObserver {
  const observer = new IntersectionObserver(
    (entries) => {
      if (entries[0]) {
        config.callBackFunction(entries[0].isIntersecting);
      }
    },
    {
      root: config.root,
