import { Directive, inject, InjectionToken, input, OnInit, output } from '@angular/core';
import { catchError, filter, merge, Observable, of, scan, switchMap, tap } from 'rxjs';
import { toObservable } from '@angular/core/rxjs-interop';
import { BecomeVisibleDirective } from './become-visible-directive';
import { outputToObservable } from '@angular/core/rxjs-interop';

export const FETCH_DATA = new InjectionToken<(filter: any, slot?: number) => Observable<any[]>>(
  'USER_ID',
);
...
export class FetchDataDirective<T> implements OnInit {
  ...
  private slot = 0;
  fetchDisabed = input(false);
  filterParams = input<any>({});
  accumulatorCallBack = input<(acc: any[], curr: any[]) => any[]>((acc: any[], curr: any[]) => [
    ...acc,
    ...curr,
  ]);
  hasMoreCallBack = input<(value: any[]) => boolean>((value: any[]) => !!value.length);
